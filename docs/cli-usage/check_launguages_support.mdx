...
Putting the fact that <a href="https://langsync.app" target="_blank">LangSync</a> is using AI models & engines in order to get you the best results, all ISO 939-1 code languages are supported and you can work with them just fine. However, <a href="https://langsync.app" target="_blank">LangSync</a> is not a magic tool that will know all imaginary or fantasy languages, and so, in order to confirm if <a href="https://langsync.app" target="_blank">LangSync</a> knows them, you will need to [check yourself](#check-languages-support)

## Check Languages Support

In order to check if <a href="https://langsync.app" target="_blank">LangSync</a> recognize a single or set of Launguage(s), all you need to do is run the following command:
...
After <a href="https://langsync.app" target="_blank">LangSync</a> checks for each language support, it will show the check result like this:
